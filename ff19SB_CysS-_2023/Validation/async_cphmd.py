...
#    if step == 0:
#        re.do_replica_exchange(phs, 1, 'template_equil_2.{}.mdin'.format(step+1), 'cys.parm7', 'ff19sb_pme.parm', 'phmdin_restart', 'equil_2.{}'.format(step+1), initial_restart_file = 'equil_1.{}.rst7'.format(first_equil_stage))
#    else:
#        re.do_replica_exchange(phs, 1, 'template_equil_2.{}.mdin'.format(step+1), 'cys.parm7', 'ff19sb_pme.parm', 'phmdin_restart', 'equil_2.{}'.format(step+1), restart_directory = 'equil_2.{}'.format(step), restart = True)
#    print("Equilibration stage step {} finished.".format(step+1))

# For production
prod_stage = 10

for step in range(0, prod_stage):
    if step == 0:

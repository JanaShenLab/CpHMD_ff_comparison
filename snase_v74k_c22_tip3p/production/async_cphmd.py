...
for step in range(2, prod_stage):
    if step == 0:
        re.do_replica_exchange(phs, 1000, 'template_prod.mdin', '3ruz_charmm.prmtop', 'charmm_pme.parm', 'prod.phmdin', 'output_stage{}'.format(step+1), concatenate_parm_file='3ruz_prot_solv_ion_charmm.psf', restart_directory = 'equil_2.{}'.format(second_equil_stage), restart = True)
    else:
        re.do_replica_exchange(phs, 1000, 'template_prod.mdin', '3ruz_charmm.prmtop', 'charmm_pme.parm', 'prod.phmdin', 'output_stage{}'.format(step+1), concatenate_parm_file='3ruz_prot_solv_ion_charmm.psf',restart_directory = 'output_stage{}'.format(step), restart = True)

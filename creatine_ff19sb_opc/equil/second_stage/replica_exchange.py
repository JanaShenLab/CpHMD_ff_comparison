    ...
    if swap_count < swap_attempts:
        ...
        with open('{}/out.log'.format(output_directory), 'a') as log_file, \
            ...
            if swap_count == 0 and not restart:
                subprocess.call(['{}/bin/pmemd.cuda'.format(amberdir), '-O', '-i',
...
def append_files(md_set, cuda_devices, phs, replica_steps, output_directory, concatenate_parm_file, template_mdin):

...
for step in range(prod_stage):
    if step == 0:
        re.do_replica_exchange(phs, 1000, 'template_prod.mdin', 'lys.parm7', 'ff19sb_pme.parm', 'prod.phmdin', 'output_stage{}'.format(step+1), restart_directory = 'equil_2.{}'.format(second_equil_stage), restart = True)
    else:
        re.do_replica_exchange(phs, 1000, 'template_prod.mdin', 'lys.parm7', 'ff19sb_pme.parm', 'prod.phmdin', 'output_stage{}'.format(step+1), restart_directory = 'output_stage{}'.format(step), restart = True)

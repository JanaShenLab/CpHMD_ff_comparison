...
for step in range(0, second_equil_stage):
    if step == 0:
        re.do_replica_exchange(phs, 1, 'template_equil_2.{}.mdin'.format(step+1), '6wgn_cphmd.prmtop', 'ff19sb_pme_AarionC_CraigK.parm', 'prod.phmdin', 'equil_2.{}'.format(step+1), initial_restart_file = 'equil_1.{}.rst7'.format(first_equil_stage))
    else:
        re.do_replica_exchange(phs, 1, 'template_equil_2.{}.mdin'.format(step+1), '6wgn_cphmd.prmtop', 'ff19sb_pme_AarionC_CraigK.parm', 'prod.phmdin', 'equil_2.{}'.format(step+1), restart_directory = 'equil_2.{}'.format(step), restart = True)
